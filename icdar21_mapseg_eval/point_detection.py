...
def eval_pt_detect(expected, predicted, radius_limit, *, beta=0.5, transfer_fn=None, debug=False, show_plot=False):
    ...
    for tid, dst, pid in zip(pred_to_targets[order], distances[order], order):
        ...
        dbg_print(f"Considering predicted point {pid:02d}: ({pred_x:7.1f},{pred_y:7.1f})")
        ...
        if dst > radius_limit:  # on the edge = OK here
            ...
            dbg_print(f"\tMatches gt point {tid:02d}: ({expected[tid,0]:7.1f},{expected[tid,1]:7.1f}) @ {dst:.1f}")
            if seen[tid]:
        ...
        if is_match:
            ...
            dy = (ay + prev_ay) / 2
            area += dx * dy
            dbg_print(f"\ttp:{tp:3d}, fn:{fn:3d}, fp:{fp:3d}, |expt|:{len(expected)}, |pred|:{len(predicted)}," 
...
def show_predictions(expected, predicted, radius_limit, filename=None):
    ...
    for xy in expected:
        c = plt.Circle(xy, radius_limit, fill=False, linestyle="--", color="black", alpha=0.5)
        axes.add_patch(c)
...
def show_predictions_classified(expected, df, radius_limit, filename=None):
    ...
    plt.plot(too_far["Prediction x"], too_far["Prediction y"], 'xk', label="predicted (too far)", alpha=0.8)
    plt.plot(extra["Prediction x"], extra["Prediction y"], 'xr', label="predicted (extra)", alpha=0.8)
    plt.plot(matches["Prediction x"], matches["Prediction y"], 'xb', label="predicted (match)", alpha=0.8)
    axes.set_aspect(1)
    plt.legend()
    if filename is not None:
        plt.savefig(filename)
